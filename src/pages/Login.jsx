...
function Login() {
  ...
  const handleLogin = async (e) => {
    ...
    const isRegisteredValid = storedUser && 
      ...
      <div style={{ background: 'white', borderRadius: '24px', padding: '48px 40px', width: '100%', maxWidth: '440px', boxShadow: '0 20px 40px -15px rgba(0,0,0,0.05)', border: '1px solid rgba(226, 232, 240, 0.8)' }}>
        ...
        <div style={{ textAlign: 'center', marginBottom: '32px' }}>
          ...
          <p style={{ color: '#64748b', fontSize: '0.95rem' }}>
            Sign in as <strong style={{color: '#0ea5e9'}}>{selectedRole.charAt(0).toUpperCase() + selectedRole.slice(1)}</strong>
          </p>
        </div>

        <form onSubmit={handleLogin} style={{ display: 'flex', flexDirection: 'column', gap: '20px', width: '100%' }}>
          <div style={{ display: 'flex', flexDirection: 'column', gap: '8px' }}>
            <label style={{ fontSize: '0.875rem', fontWeight: '500', color: '#475569' }}>Email or Username</label>
            <input
              type="text"
              ...
              required
              style={{ width: '100%', padding: '14px 16px', borderRadius: '12px', border: '1px solid #cbd5e1', backgroundColor: '#f8fafc', fontSize: '0.95rem', outline: 'none', transition: 'border-color 0.2s' }}
          ...
          <button type="submit" style={{ width: '100%', padding: '16px', backgroundColor: '#0ea5e9', color: 'white', border: 'none', borderRadius: '12px', fontSize: '1rem', fontWeight: '600', cursor: 'pointer', transition: 'background-color 0.2s' }} onMouseOver={(e) => e.target.style.backgroundColor = '#0284c7'} onMouseOut={(e) => e.target.style.backgroundColor = '#0ea5e9'}>
            Sign In
          </button>
        </form>
