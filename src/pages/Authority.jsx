...
<div className="chart-container">
  <h3 className="chart-title">🏥 Hospital Comparison Tool</h3>
  <p style={{ color: '#64748b', fontSize: '0.9rem', marginBottom: '20px' }}>Select two hospitals to compare them side-by-side.</p>

  <div style={{ display: 'flex', gap: '16px', marginBottom: '24px', flexWrap: 'wrap' }}>
    <select
      value={compareA}
      onChange={e => setCompareA(e.target.value)}
      style={{ flex: 1, minWidth: '200px', padding: '10px 14px', borderRadius: '8px', border: '2px solid #3b82f6', fontSize: '0.95rem', outline: 'none' }}
    >
      <option value="">-- Select Hospital A --</option>
      {hospitals.map(h => <option key={h.id} value={h.id}>{h.name}</option>)}
    </select>
    <div style={{ display: 'flex', alignItems: 'center', fontWeight: '800', fontSize: '1.2rem', color: '#64748b' }}>VS</div>
    <select
      value={compareB}
      onChange={e => setCompareB(e.target.value)}
      style={{ flex: 1, minWidth: '200px', padding: '10px 14px', borderRadius: '8px', border: '2px solid #f59e0b', fontSize: '0.95rem', outline: 'none' }}
    >
      <option value="">-- Select Hospital B --</option>
      {hospitals.map(h => <option key={h.id} value={h.id}>{h.name}</option>)}
    </select>
    {(compareA || compareB) && (
      <button onClick={() => { setCompareA(''); setCompareB(''); }} style={{ padding: '10px 18px', backgroundColor: '#f1f5f9', border: '1px solid #e2e8f0', borderRadius: '8px', cursor: 'pointer', color: '#64748b', fontWeight: '600' }}>Reset</button>
    )}
  </div>

  {(() => {
    const hA = hospitals.find(h => h.id === compareA)
    const hB = hospitals.find(h => h.id === compareB)
    if (!hA && !hB) return <div style={{ textAlign: 'center', padding: '40px', color: '#94a3b8', border: '2px dashed #e2e8f0', borderRadius: '12px' }}>👆 Select two hospitals above to compare</div>

    const getOccupancy = h => {
      const t = parseInt(h?.totalBeds) || 0
      const a = parseInt(h?.availableBeds) || 0
      return t > 0 ? Math.round(((t - a) / t) * 100) : 0
    }
    const getTotalCases = h => {
      if (!h) return 0
      let total = (parseInt(h.dengueCases) || 0) + (parseInt(h.fluCases) || 0)
      if (h.customDiseases) h.customDiseases.forEach(d => { total += parseInt(d.cases) || 0 })
      return total
    }
    const getAllDiseaseNames = () => {
      const names = new Set()
      if (hA?.dengueCases > 0) names.add('Dengue')
      if (hA?.fluCases > 0) names.add('Flu')
      hA?.customDiseases?.forEach(d => d.name && names.add(d.name.trim()))
      if (hB?.dengueCases > 0) names.add('Dengue')
      if (hB?.fluCases > 0) names.add('Flu')
      hB?.customDiseases?.forEach(d => d.name && names.add(d.name.trim()))
      return Array.from(names)
    }
    const getDiseaseCount = (h, dName) => {
      if (!h) return 0
      if (dName.toLowerCase() === 'dengue') return h.dengueCases || 0
      if (dName.toLowerCase() === 'flu') return h.fluCases || 0
      return h.customDiseases?.find(d => d.name.trim().toLowerCase() === dName.toLowerCase())?.cases || 0
    }

    const occA = getOccupancy(hA), occB = getOccupancy(hB)
    const casesA = getTotalCases(hA), casesB = getTotalCases(hB)
    const trustA = Math.round(hA?.trustScore || 100), trustB = Math.round(hB?.trustScore || 100)
    const diseaseNames = getAllDiseaseNames()

    const colA = '#3b82f6', colB = '#f59e0b'
    const better = (a, b, lowerIsBetter = false) => {
      if (a === undefined || a === null) return { a: '', b: '' }
      if (a === b) return { a: '', b: '' }
    ...
    const rows = [
      { label: '📍 Location', a: hA?.location || 'N/A', b: hB?.location || 'N/A', compare: false },
      { label: '🛏️ Total Beds', a: hA?.totalBeds || 0, b: hB?.totalBeds || 0, compare: true },
      { label: '✅ Available Beds', a: hA?.availableBeds || 0, b: hB?.availableBeds || 0, compare: true },
      { label: '📊 Occupancy Rate', a: occA + '%', b: occB + '%', badge: better(occA, occB, true), isOccupancy: true, rawA: occA, rawB: occB },
      { label: '🦠 Total Active Cases', a: casesA, b: casesB, compare: true, lowerBetter: true },
      { label: '🔬 Ventilators', a: hA?.currentEquipment?.ventilators || 0, b: hB?.currentEquipment?.ventilators || 0, compare: true },
      { label: '🛡️ ML Trust Score', a: trustA + '%', b: trustB + '%', badge: better(trustA, trustB), rawA: trustA, rawB: trustB },
      { label: '⚠️ Fraud Status', a: hA?.fraudStatus || 'Normal', b: hB?.fraudStatus || 'Normal', compare: false },
      ...diseaseNames.map(d => ({ label: `🔴 ${d} Cases`, a: getDiseaseCount(hA, d), b: getDiseaseCount(hB, d), compare: true, lowerBetter: true }))
    ]
        ...
        <table style={{ width: '100%', borderCollapse: 'collapse', fontSize: '0.95rem' }}>
          <thead>
            <tr style={{ backgroundColor: '#f8fafc' }}>
              <th style={{ padding: '14px 16px', textAlign: 'left', color: '#475569', fontWeight: '700', width: '30%', borderBottom: '2px solid #e2e8f0' }}>Metric</th>
              <th style={{ padding: '14px 16px', textAlign: 'center', borderBottom: `3px solid ${colA}`, color: colA, fontWeight: '800', width: '35%' }}>
                🏥 {hA ? hA.name : '—'}
              </th>
              <th style={{ padding: '14px 16px', textAlign: 'center', borderBottom: `3px solid ${colB}`, color: colB, fontWeight: '800', width: '35%' }}>
                🏥 {hB ? hB.name : '—'}
              </th>
            </tr>
          </thead>
          <tbody>
            {rows.map((row, i) => {
              const badge = row.badge || (row.compare ? better(typeof row.a === 'string' ? parseInt(row.a) : row.a, typeof row.b === 'string' ? parseInt(row.b) : row.b, row.lowerBetter) : { a: '', b: '' })
              const bgA = badge.a === '✅' ? '#f0fdf4' : badge.a === '❌' ? '#fff1f2' : 'transparent'
              const bgB = badge.b === '✅' ? '#f0fdf4' : badge.b === '❌' ? '#fff1f2' : 'transparent'
              return (
                ...
                >
                  ...
                  <td style={{ padding: '14px 16px', textAlign: 'center', backgroundColor: bgA, fontWeight: '600', color: '#1e293b' }}>
                    {hA ? <>{row.a} {badge.a && <span style={{ marginLeft: '6px' }}>{badge.a}</span>}</> : <span style={{ color: '#cbd5e1' }}>—</span>}
                    {row.isOccupancy && hA && (
                      <div style={{ marginTop: '6px', backgroundColor: '#e2e8f0', height: '6px', borderRadius: '4px', overflow: 'hidden' }}>
                        <div style={{ width: `${row.rawA}%`, height: '100%', borderRadius: '4px', backgroundColor: row.rawA > 80 ? '#ef4444' : row.rawA > 50 ? '#f59e0b' : '#10b981' }} />
                      </div>
                    )}
                  </td>
                  <td style={{ padding: '14px 16px', textAlign: 'center', backgroundColor: bgB, fontWeight: '600', color: '#1e293b' }}>
                    {hB ? <>{row.b} {badge.b && <span style={{ marginLeft: '6px' }}>{badge.b}</span>}</> : <span style={{ color: '#cbd5e1' }}>—</span>}
                    {row.isOccupancy && hB && (
                      <div style={{ marginTop: '6px', backgroundColor: '#e2e8f0', height: '6px', borderRadius: '4px', overflow: 'hidden' }}>
                        <div style={{ width: `${row.rawB}%`, height: '100%', borderRadius: '4px', backgroundColor: row.rawB > 80 ? '#ef4444' : row.rawB > 50 ? '#f59e0b' : '#10b981' }} />
                      </div>
                    )}
                  </td>
                </tr>
